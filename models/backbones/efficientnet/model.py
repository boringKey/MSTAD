...
class EfficientNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # # # Convolution layers
        # x_mask, feat_dict_mask, _ = self.extract_features(image_mask)
        # # Pooling and final linear layer
        # x_mask = self._avg_pooling(x_mask)
        # if self._global_params.include_top:
        #     x_mask = x_mask.flatten(start_dim=1)
        #     x_mask = self._dropout(x_mask)
        #     x_mask = self._fc(x_mask)
        #
        # features_mask = [feat_dict_mask[i] for i in self.outblocks]
